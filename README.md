...
This is an Opencode plugin that adds Antigravity (Cloud Code) OAuth support for Google Gemini and Claude models. Follow the steps below to install and authenticate.

### 0) Create a Google OAuth client (one-time setup)
...
### 4) Callback behavior (good to know)
...
1. Install the plugin locally as above or via the registry.
